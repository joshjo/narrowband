...
PYTHON() void adjustNumber(
	BasicParticleSystem& parts, const MACGrid& vel, const FlagGrid& flags,
	int minParticles, int maxParticles, const LevelsetGrid& phi, Real radiusFactor=1.,
	Real narrowBand=-1., const Grid<Real>* exclude=NULL)
{
                                 ...
                                 const FlagGrid& flags, Grid<int>& index, Grid<int>* counter=NULL )
{
	...
	if(!counter) {
		counter = new Grid<int>(  flags.getParent() ); delCounter=true;
	} else {
		counter->clear();
	}
...
void fixParticles(
	...
	// , Real radiusFactor
	// const Grid<Real>& gird
	// , Real factor
) {
	const Real radius = 0.5 * calculateRadiusFactor(phi, radiusFactor);
	// std::cout << indexSys.size() << std::endl;
	std::cout << "Index size: " << index.getSize() << std::endl;
	std::cout << "Flags size: " << flags.getSize() << std::endl;
	std::cout << "Parts size: " << parts.size() << std::endl;
	std::cout << "Phi size: " << phi.getSize() << std::endl;
	std::cout << "radius: " << radius << std::endl;

	// printf("index size: %d", indexSys.size());
	// const Real radius = 0.5 * calculateRadiusFactor(phi, radiusFactor);
